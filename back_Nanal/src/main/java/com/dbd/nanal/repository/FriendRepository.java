...
public interface FriendRepository extends JpaRepository<FriendEntity, Integer> {
    ...
    @Query(nativeQuery = true, value = "select distinct substring_index(substring_index(substring_index(d.emo,'/',-1),'_',2),'_',-1) as emo, d.user_idx, p.nickname from diary d join user_profile p on d.user_idx = p.user_idx where d.user_idx in (select f.user_idx2 from friend f where user_idx = 5) and d.diary_date= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY),'%Y-%m-%d')")
    List<Object[]> findEmotionOfLastDay(int userIdx);
}
